...
- Improved handling of expired or revoked Amazee.ai credentials in the AI
  recovery path: the call now degrades cleanly and the site is flagged for
  admin re-authentication, and AI health status more accurately reflects
  credential state. The model-resolution self-heal is unchanged.
